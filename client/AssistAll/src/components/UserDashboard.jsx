...
const StatusBanner = ({ status }) => {
    ...
    return <div className={`fixed top-0 left-0 right-0 ${bg} text-white px-4 py-3 z-[5000] shadow-xl animate-in slide-in-from-top flex items-center justify-center gap-3`}>{icon} <span className="font-bold text-xs tracking-widest">{text}</span></div>;
};
...
const FindingVolunteer = ({ requestId, onCancel }) => {
    ...
    const handleCancel = async () => {
        if (requestId) {
            try {
                await fetch(`${API_BASE}/api/requests/${requestId}/cancel`, { method: 'PUT', headers: { "Content-Type": "application/json" } });
    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 z-[2000] bg-white rounded-t-3xl shadow-2xl p-8 pb-12 animate-in slide-in-from-bottom">
            <div className="flex flex-col items-center text-center mt-4">
                ...
                <button onClick={handleCancel} className="w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center hover:bg-red-100 text-red-600 transition shadow-md"><X size={24}/></button>
...
const RideInProgress = ({ requestData }) => {
    ...
    return (
        <div className={`absolute left-0 right-0 bg-[#121212] rounded-t-[30px] shadow-2xl z-50 border-t border-white/10 transition-all duration-500 ease-in-out ${isExpanded ? 'bottom-0 pb-24 h-[60vh]' : 'bottom-0 h-[180px]'}`}>
            ...
            <div className="px-6">
                <div className="flex justify-between items-center mb-6"><div><div className="flex items-center gap-2 mb-1"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span></span><p className="text-green-500 text-xs font-bold uppercase tracking-widest">ON TRIP</p></div><h2 className="text-3xl font-black text-white">{isExpanded ? "Your Ride" : requestData.volunteerName}</h2></div><button className="w-10 h-10 bg-neutral-800 rounded-full flex items-center justify-center border border-white/10 text-white"><Share2 size={18}/></button></div>
                <div className={`transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 hidden'}`}><div className="bg-[#1a1a1a] p-5 rounded-3xl border border-white/5 mb-6"><div className="flex items-start mb-6"><div className="w-3 h-3 bg-white rounded-full border-2 border-neutral-500 shadow mr-4 mt-1"></div><div><p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Pickup</p><p className="font-bold text-gray-200 text-sm">{requestData.pickup || "Current Location"}</p></div></div><div className="flex items-start"><div className="w-3 h-3 bg-green-500 rounded-full border-2 border-white shadow mr-4 mt-1"></div><div><p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Destination</p><p className="font-bold text-gray-200 text-sm">{requestData.drop}</p></div></div></div></div>
            </div>
        </div>
    );
};
...
const RateAndTip = ({ requestData, onSkip, onSubmit }) => {
    ...
    const handleFinalSubmit = async (method) => {
        ...
        try {
            ...
            addToast("Thank you! Feedback Submitted.", "success");
            onSubmit(); 
        } catch (err) { onSubmit(); } 
...
const UserDashboard = () => {
    ...
    const handleRequest = async (requestDetails) => {
        try {
            ...
            const res = await fetch(`${API_BASE}/api/requests`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requesterName: user?.name || "User", requesterId: user?._id, type: requestDetails.type, price: 150, pickup: "Kottayam", drop: requestDetails.dropOff }) });
            if (res.ok) {
                const data = await res.json();
                localStorage.setItem('activeRideId', data._id);
                setRideIdToPoll(data._id);
                setViewState('searching');
                addToast("Searching for volunteers...", "info");
            }
    ...
    const handleRatingComplete = () => {
        ...
        setActiveRide(null);
        setViewState('menu');
        addToast("Thanks for using AssistAll!", "success");
    };
    ...
    return (
        <div className="h-screen bg-neutral-100 text-black font-sans flex flex-col relative overflow-hidden">
            ...
            {viewState === 'searching' && <FindingVolunteer requestId={rideIdToPoll} onCancel={handleCancelRequest} />}
