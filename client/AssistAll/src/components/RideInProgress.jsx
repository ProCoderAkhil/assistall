      ...
      <div className="px-6">
          ...
          <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                  <button onClick={handleShare} className="w-10 h-10 bg-blue-900/30 rounded-full flex items-center justify-center border border-blue-500/50 text-blue-400"><Share2 size={18}/></button>
                  <button onClick={handleReport} className={`w-10 h-10 rounded-full flex items-center justify-center border ${reported ? 'bg-red-600 text-white' : 'bg-red-900/20 border-red-500/50 text-red-500'}`}><AlertTriangle size={18}/></button>
              </div>
          </div>
          ...
          <div className={`transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 hidden'}`}>
              ...
              <div className="bg-blue-900/10 p-4 rounded-xl border border-blue-500/20 flex items-center gap-3">
                  ...
                  <div><p className="text-white font-bold text-sm">Ride Monitored</p><p className="text-blue-300 text-xs">Admin is tracking this trip for safety.</p></div>
              </div>
          </div>
      </div>
    </div>
